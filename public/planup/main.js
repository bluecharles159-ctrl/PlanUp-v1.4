...
const SETTINGS_KEY = "planup_settings";
const SUPPORT_EMAIL = "planupsupport@gmail.com";
const COUNTRIES = [
...
function getCurrencyMeta(code = appSettings.currency) {
  return CURRENCIES[code] || CURRENCIES.USD;
}

function formatMoney(amount) {
  const n = Number(amount) || 0;
  const code = appSettings.currency || "USD";
  const meta = getCurrencyMeta(code);
  const shouldConvert = appSettings.convertCurrency && code !== "USD";
  const converted = shouldConvert ? n * (fxRates[code] || FALLBACK_RATES[code] || 1) : n;
  const noCents = code === "JPY" || code === "KRW";
  const value = noCents ? Math.round(converted) : Math.round(converted * 100) / 100;
...
function refreshMoneyDisplays() {
  if (typeof updateBudgetProgress === "function") updateBudgetProgress(true);
  if (typeof updateInsightsPage === "function") updateInsightsPage();
  if (typeof renderSavedItems === "function") renderSavedItems();
  if (typeof renderHistory === "function") renderHistory();
  if (typeof renderConcurrentItems === "function") renderConcurrentItems();
  if (typeof renderToGetItems === "function") renderToGetItems();
  if (typeof renderInsightsChart === "function") renderInsightsChart();
}
...
function updateInsightsCategoryBreakdown(categoryStats, totalOverallSpent) {
  ...
  if (topByQty) {
    ...
    mostItemsCard.innerHTML = `
      ...
      <div class="categorySpending">${formatMoney(topByQty.totalPrice)}</div>
  ...
  if (topBySpending && topBySpending.name !== topByQty?.name) {
    ...
    mostSpentCard.innerHTML = `
      ...
      <div class="categorySpending">${formatMoney(topBySpending.totalPrice)}</div>
      ...
      scales: {
        ...
        y: {
          ...
          ticks: {
            font: { size: 13 },
            color: "#475569",
            callback: (value) => (isAmountMode ? formatMoney(value) : value),
...
function scaleIngredientLine(text, fromServings, toServings) {
  const from = Number(fromServings) || 0;
  const to = Number(toServings) || 0;
  if (!from || !to || from === to) return text;
  const factor = to / from;
  return String(text).replace(/(\d+\.?\d*)/g, (match) => {
    const scaled = parseFloat(match) * factor;
    if (Number.isNaN(scaled)) return match;
...
function updateMakeRecipePage(recipe) {
  ...
  const recipeServings = parseInt(recipe.servings, 10) || 4;
  const targetServings = parseInt(appSettings.servingSize, 10) || recipeServings;
  ...
  if (makeRecipeTechnique)
    makeRecipeTechnique.textContent = recipe.category
      ...
      : "Cooking";
  if (makeRecipeServings) makeRecipeServings.textContent = String(targetServings);
  ...
  if (makeRecipeIngredientsList) {
    makeRecipeIngredientsList.innerHTML = (recipe.ingredients || [])
      .map(
        (ingredient) =>
          `<li>${safeText(scaleIngredientLine(ingredient, recipeServings, targetServings))}</li>`,
      )
      .join("");
  }
...
function openMakeRecipePage(recipe) {
  ...
  if (typeof syncCookModeWakeLock === "function") syncCookModeWakeLock();
}
...
document.querySelectorAll(".toggler").forEach((toggler) => {
  ...
  if (managedIds.has(toggler.id)) return;
...
currencyOptions.forEach((option) => {
  option.addEventListener("click", (e) => {
    e.stopPropagation();
    const code = option.dataset.code || option.textContent.trim().split(" ")[0];
    appSettings.currency = CURRENCIES[code] ? code : "USD";
    saveSettings();
    syncCurrencyLabel();
    currencyModal.classList.remove("show");
    currencyOverlay.classList.remove("show");
    if (profilePage) profilePage.style.transform = "scale(1)";
    refreshMoneyDisplays();
    showToast(`Currency set to ${appSettings.currency}`);
  });
});
...
function closePage() {
  document.querySelectorAll(".menu-xrn.show").forEach((p) => {
  ...
  div.innerHTML = `
    ...
    <div class="to-get-item-details">
      ...
      <div class="to-get-item-meta">
        ...
        <span>${item.price ? `Price: ${formatMoney(item.price)}` : ""}</span>
...
// ===== Unified menu-xrn navigation =====
// Opens a full page (menu-xrn) so it slides out from behind the menu
// bar/drawer, and closes any other open menu-xrn first so only one is
// ever visible at a time.
function openMenuXrn(page) {
  if (!page) return;
  document.querySelectorAll(".menu-xrn.show").forEach((p) => {
    if (p !== page) p.classList.remove("show");
  });
...
if (toGetListBtn) {
  toGetListBtn.addEventListener("click", () => {
    openMenuXrn(toGetListPage);
  });
}

if (favoritesBtn) {
  favoritesBtn.addEventListener("click", () => {
    openMenuXrn(favoritesPage);
  });
}

if (historyBtn) {
  historyBtn.addEventListener("click", () => {
    openMenuXrn(historyPage);
  });
}
...
if (scheduledBtn) {
  scheduledBtn.addEventListener("click", () => {
    openMenuXrn(scheduledPage);
  });
}

if (feedbackBtn) {
  feedbackBtn.addEventListener("click", () => {
    openMenuXrn(feedbackPage);
  });
}
...
function sortAllCategories() {
  ...
  const categoriesData = categoryCards.map((card) => {
    ...
    const totalPrice = parseFloat(String(totalPriceText).replace(/[^\d.-]/g, "")) || 0;
...
function renderHistory() {
  ...
  hist.forEach((h) => {
    ...
    div.innerHTML = `
      ...
      <div class="meta">${h.quantity} • ${formatMoney(h.price || 0)}</div>
    `;
      ...
      div.innerHTML = `
        ...
        <div class="concurrent-items-details">
          <div class="concurrent-detail-row">
            ...
            <div class="concurrent-item-detail">Price: ${formatMoney(item.price)}</div>
          </div>
          <div class="concurrent-detail-row">
...
function updateBudgetProgress(silent = false) {
  ...
  // Update DOM
  if (estimatesValue) estimatesValue.textContent = formatMoney(totalSpent);
  if (remainingValue)
    remainingValue.textContent = formatMoney(Math.max(0, remaining));
  if (avgSpending) avgSpending.textContent = formatMoney(avgPerItem);
  ...
  if (percentage < 70) {
    ...
    } else if (silent && percentage >= alertAtBudget) {
      // Don't fire a bell alert just because the page loaded in an
      // already-over-threshold state; just mark it as acknowledged.
      window.__budgetAlertNotified = true;
    }
  } else {
    ...
    if (!silent && budgetAlertsEnabled && !window.__budgetAlertNotified) {
      showBudgetNotification(
        `You've used ${Math.round(percentage)}% of your budget`,
      );
      window.__budgetAlertNotified = true;
...
function showNotification(options) {
  ...
  // De-duplicate: the same message inside this window is treated as the
  // *same* message, so re-rendering a page or re-running a recalculation
  // can never make the bell ring again.
  const DEDUPE_MS = 60000;
  const now = Date.now();
  window.__lastNotification = window.__lastNotification || {
    message: null,
    time: 0,
  };
  if (
    window.__lastNotification.message === message &&
    now - window.__lastNotification.time < DEDUPE_MS
  ) {
    return;
  }
  window.__lastNotification = { message, time: now };

  // Record the message first: the bell only ever animates as a reaction to
  // this new entry, never as a side effect of navigation or layout work.
  const notificationId = now;
  ...
  const ring = () => {
    ...
    // Restart the shake cleanly so a rapid second message still animates.
    notificationBtn.classList.remove("shake");
    void notificationBtn.offsetWidth;
    notificationBtn.classList.add("shake");
    if (window.__bellShakeTimer) clearTimeout(window.__bellShakeTimer);
    window.__bellShakeTimer = setTimeout(
      () => notificationBtn.classList.remove("shake"),
      500,
    );
    ...
    if ("Notification" in window && Notification.permission === "granted") {
      try {
        new Notification("PlanUP", { body: message });
      } catch {
        /* ignore */
      }
    }
    ...
    if (autoHide) {
      if (window.__notificationHideTimer)
        clearTimeout(window.__notificationHideTimer);
      window.__notificationHideTimer = setTimeout(() => hideNotification(), 5000);
    }
  };

  if (delay > 0) {
    setTimeout(ring, delay);
  } else {
    ring();
  }
}
...
function setTogglerState(toggler, on) {
  if (!toggler) return;
  toggler.querySelector(".toggle")?.classList.toggle("on", on);
  toggler.querySelector(".toggle-ground")?.classList.toggle("on", on);
}

function syncCurrencyLabel() {
  const meta = getCurrencyMeta();
  if (currencyDiv) currencyDiv.textContent = `${meta.symbol} ${meta.name}`;
}
...
function syncCountryLabel() {
  ...
  if (countrySelect) countrySelect.textContent = countryByCode(appSettings.country).name;
}

async function syncCookModeWakeLock() {
  const recipeOpen = makeRecipePage?.classList.contains("show");
  const shouldLock = appSettings.cookMode && recipeOpen;
  if (!shouldLock) {
    if (cookWakeLock) {
      try {
        await cookWakeLock.release();
      } catch {
        /* ignore */
      }
      cookWakeLock = null;
  ...
  if (!("wakeLock" in navigator)) {
    showToast("Cook mode isn't supported in this browser");
    return;
  }
  try {
    cookWakeLock = await navigator.wakeLock.request("screen");
    cookWakeLock.addEventListener("release", () => {
      cookWakeLock = null;
    });
  } catch {
    showToast("Couldn't keep the screen awake");
  }
}
...
function exportGroceryList() {
  const toGet =
    typeof getToGetItems === "function" ? getToGetItems() : [];
  const rows = [
    ["Name", "Category", "Quantity", "Price", "Store", "Added"].join(","),
    ...items.map((item) =>
      [
        item.name,
        item.category,
  ...
  downloadTextFile("planup-grocery-list.csv", rows.join("\n"), "text/csv");
  downloadTextFile(
    "planup-backup.json",
    JSON.stringify(
      {
...
function openAppPicker(title, choices, onPick) {
  const overlay = document.getElementById("appPickerOverlay");
  const picker = document.getElementById("appPicker");
  const pickerTitle = document.getElementById("appPickerTitle");
  const list = document.getElementById("appPickerList");
  if (!overlay || !picker || !list) return;
  pickerTitle.textContent = title;
  list.innerHTML = "";
  choices.forEach((choice) => {
    const btn = document.createElement("button");
    btn.type = "button";
    btn.className = "app-picker-option";
    btn.textContent = choice.label;
    if (choice.selected) btn.classList.add("selected");
...
function closeAppPicker() {
  document.getElementById("appPickerOverlay")?.classList.remove("show");
  document.getElementById("appPicker")?.classList.remove("show");
}

function sendBugReport() {
  const details = document.getElementById("bugReportInput")?.value.trim();
  if (!details) {
    showToast("Describe the bug first");
    return;
  }
  const subject = encodeURIComponent("PlanUP bug report");
  const body = encodeURIComponent(
    `Bug report from PlanUP\n\n${details}\n\n---\nTheme: ${appSettings.theme}\nCountry: ${appSettings.country}\nCurrency: ${appSettings.currency}\nConvert currency: ${appSettings.convertCurrency}\nCook mode: ${appSettings.cookMode}\nServings: ${appSettings.servingSize}\nItems: ${items.length}\nApp: PlanUp 1.0.0\nPage: ${location.href}\nTime: ${new Date().toISOString()}`,
  );
...
async function fetchFxRates() {
  try {
    ...
    fxRates = { USD: 1, ...FALLBACK_RATES, ...(data.rates || {}) };
    if (appSettings.convertCurrency) refreshMoneyDisplays();
  } catch {
    fxRates = { ...FALLBACK_RATES };
  }
}

function bindSettingsToggler(id, getValue, setValue) {
  const toggler = document.getElementById(id);
  if (!toggler) return;
  setTogglerState(toggler, getValue());
  toggler.querySelector(".click-listener")?.addEventListener("click", () => {
    setValue(!getValue());
  });
}
...
function initSettingsControls() {
  ...
  const servingInput = document.getElementById("servingSizeInput");
  if (servingInput) {
    servingInput.value = appSettings.servingSize;
    const saveServings = () => {
      let n = parseInt(servingInput.value, 10);
      if (isNaN(n) || n < 1) n = 1;
      if (n > 99) n = 99;
      servingInput.value = n;
      appSettings.servingSize = n;
      saveSettings();
      showToast(`Default servings set to ${n}`);
    };
    servingInput.addEventListener("change", saveServings);
  }

  document.getElementById("themeSelect")?.addEventListener("click", () => {
    openAppPicker(
      "Theme",
      ["light", "dark", "system"].map((value) => ({
        value,
        label: value.charAt(0).toUpperCase() + value.slice(1),
        selected: appSettings.theme === value,
      })),
      (value) => {
        appSettings.theme = value;
        saveSettings();
        applyTheme();
        showToast(`Theme: ${value}`);
      },
    );
  });
  ...
  document.getElementById("countrySelect")?.addEventListener("click", () => {
    openAppPicker(
      ...
      COUNTRIES.map((country) => ({
        ...
        selected: appSettings.country === country.code,
      })),
      (code) => {
        const country = countryByCode(code);
        appSettings.country = country.code;
        appSettings.currency = country.currency;
        saveSettings();
        syncCountryLabel();
        syncCurrencyLabel();
        refreshMoneyDisplays();
        showToast(`Region set to ${country.name}`);
      },
    );
  });
  ...
  bindSettingsToggler(
    "cookModeToggler",
    () => appSettings.cookMode,
    (on) => {
      appSettings.cookMode = on;
      saveSettings();
      setTogglerState(document.getElementById("cookModeToggler"), on);
      syncCookModeWakeLock();
      showToast(on ? "Cook mode on" : "Cook mode off");
    },
  );
  ...
  bindSettingsToggler(
    ...
    (on) => {
      appSettings.convertCurrency = on;
      if (on && appSettings.currency === "USD") {
        showToast("Pick a local currency to convert into");
        currencyModal?.classList.add("show");
        currencyOverlay?.classList.add("show");
      }
      saveSettings();
      setTogglerState(document.getElementById("currencyConverterToggler"), on);
      refreshMoneyDisplays();
      showToast(on ? "Currency conversion on" : "Showing original amounts");
    },
  );
  ...
  bindSettingsToggler(
    ...
    async (on) => {
      ...
      appSettings.notificationsEnabled = on;
      saveSettings();
      setTogglerState(document.getElementById("pushNotificationToggler"), on);
  ...
  bindSettingsToggler(
    ...
    (on) => {
      appSettings.privacyLock = on;
      saveSettings();
      setTogglerState(document.getElementById("securityToggler"), on);
      showToast(on ? "Privacy lock enabled" : "Privacy lock off");
    },
  );

  document.getElementById("settingsExportAction")?.addEventListener("click", exportGroceryList);
  document.getElementById("exportAction")?.addEventListener("click", exportGroceryList);
  document.getElementById("sendBugReportBtn")?.addEventListener("click", sendBugReport);
  document.getElementById("appPickerOverlay")?.addEventListener("click", closeAppPicker);

  document.getElementById("settingsDeleteAction")?.addEventListener("click", () => {
  ...
  document.addEventListener("visibilitychange", () => {
    if (document.visibilityState === "visible") syncCookModeWakeLock();
  });
}
...
if (atPlaceText) atPlaceText.innerText = "@" + "Home";
...
if (profileForeground) {
  const COLLAPSE_DISTANCE = 120; // px of scroll that fully collapses the header
  let target = 0;
  let current = 0;
  let frame = null;

  const applyTravel = () => {
    // How far right the avatar may slide, measured from the live layout so
    // it always lands flush against the right edge on any screen width.
    const width = profileForeground.clientWidth || 0;
    const travel = Math.max(0, width - 90 - 20 - 16);
    profileForeground.style.setProperty("--avatar-travel", `${travel}px`);
  };

  const paint = (value) => {
    profileForeground.style.setProperty("--scroll-progress", value);
    if (profFgHeader) profFgHeader.classList.toggle("small", value > 0.55);
  };

  // Critically-damped follow: the visuals ease toward wherever the scroll
  // position points, so it feels buttery both down and back up, and never
  // snaps or fights the browser's own momentum.
  const tick = () => {
  ...
  profileForeground.addEventListener(
    "scroll",
    () => {
      target = Math.max(
        0,
        Math.min(1, profileForeground.scrollTop / COLLAPSE_DISTANCE),
      );
      schedule();
    },
    { passive: true },
  ...
  window.addEventListener("resize", applyTravel);

  // Opening Profile always starts from the top, fully expanded.
  const navProfilesBtn = document.getElementById("navProfiles");
  if (navProfilesBtn) {
    navProfilesBtn.addEventListener("click", () => {
      profileForeground.scrollTop = 0;
      target = 0;
      current = 0;
      applyTravel();
      paint(0);
...
function updateRecentItems() {
  ...
  recent.forEach((it) => {
    ...
    itemDiv.innerHTML = `
      ...
      <span class="recent-item-price">${formatMoney(parseFloat(it.price) || 0)}</span>
